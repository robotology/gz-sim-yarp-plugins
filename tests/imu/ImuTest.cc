...
TEST(ImuTest, PluginTest)
{
  ...
  {   
    readSuccessful = igyroscope->getThreeAxisGyroscopeMeasure(0, measureGyroscope, timestampGyroscope) &&
                    iorientation->getOrientationSensorMeasureAsRollPitchYaw(0, measureOrientation, timestampOrientation) &&
                    iaccelerometer->getThreeAxisLinearAccelerometerMeasure(0, measureAccelerometer, timestampAccelerometer);
